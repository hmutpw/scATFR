...
setMethod("regulonActivity", "SingleCellExperiment", function(x,
                                                              gene_list=NULL,
                                                              method = c("viper","aucell","ssgsea","gsva"),
                                                              normalization = TRUE,
                                                              min_target_num = 5L,
                                                              max_target_num = 500L,
                                                              use_assay = 1L,
  ...
  if(method == "aucell"){
    atfr <- runAUCell(exp_mat = exp_mat,
                      gene_list = gene_list,
                      normalization = normalization,
                      ncores = ncores,
                      verbose = verbose, ...)
  }else if(method == "viper"){
    atfr <- runViper(exp_mat = exp_mat,
                     gene_list = gene_list,
                     ncores = ncores,
                     normalization = normalization, ...)

  }else if(method %in% c("gsva","ssgsea")){
    atfr <- runGSVA(exp_mat = exp_mat,
                    gene_list = gene_list,
                    method = method,
  ...
  atfr_sce <- SingleCellExperiment::SingleCellExperiment(as.matrix(atfr))
...
setMethod("regulonActivity", "matrix", function(x,
                                                gene_list,
                                                method = c("viper","aucell","gsva","ssgsea","zscore", "plage"),
                                                normalization = TRUE,
                                                min_target_num = 5,
                                                max_target_num = 500,
                                                ncores = 1,
...
#' @rdname regulonActivity
#' @export
setMethod("regulonActivity", "ExpressionSet", function(x,
                                                       gene_list,
                                                       method = c("viper","aucell","gsva","ssgsea","zscore", "plage"),
                                                       normalization = TRUE,
                                                       min_target_num = 5,
  ...
  gene_list <- as.list(gene_list)
  if(method == "aucell"){
    atfr <- runAUCell(exp_mat = x,
                      gene_list = gene_list,
                      normalization = normalization,
                      ncores = ncores,
                      verbose = verbose, ...)
  }else if(method == "viper"){
    atfr <- runViper(exp_mat = x,
                     gene_list = gene_list,
                     ncores = ncores,
                     normalization = normalization, ...)
  }else if(method %in% c("gsva","ssgsea","zscore", "plage")){
    atfr <- runGSVA(exp_mat = x,
                    gene_list = gene_list,
                    method = method,
                    min_target_num = min_target_num,
                    max_target_num = max_target_num,
  ...
  eset <- Biobase::ExpressionSet(assayData = eset,
...
#' @rdname regulonActivity
#' @export
setMethod("regulonActivity", "Seurat", function(x,
                                                gene_list,
                                                method = c("viper","aucell","gsva","ssgsea","zscore", "plage"),
                                                normalization = TRUE,
                                                min_target_num = 5,
  ...
  method <- match.arg(method)
  gene_list <- as.list(gene_list)
  exp_mat <- as.matrix(Seurat::GetAssayData(x, assay = "RNA", slot = "data"))
  if(method == "aucell"){
    atfr <- runAUCell(exp_mat = exp_mat,
                      gene_list = gene_list,
                      normalization = normalization,
  ...
  #----include TF activities into Seurat object
  atfr_assay <- Seurat::CreateAssayObject(data = atfr)
  Seurat::Key(atfr_assay) <- "atfr_"
  x[["atfr_mat"]] <- atfr_assay
  x
})
